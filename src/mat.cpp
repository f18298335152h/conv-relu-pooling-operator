...
void IntelMat::substract_mean_normalize(const float* mean_vals, const float* norm_vals)
{
    int size = w * h;
...
IntelMat IntelMat::from_float16(const unsigned short* data, int size)
{
    IntelMat m(size);
    if (m.empty())
        return m;
...
static void copy_make_border_image(const IntelMat& src, IntelMat& dst, int top, int left, int type, float v)
{
    int w = dst.w;
    int h = dst.h;
...
void copy_make_border(const IntelMat& src, IntelMat& dst, int top, int bottom, int left, int right, int type, float v)
{
    ...
    else if (src.dims == 3)
    {
        ...
        {
            const IntelMat m = src.channel(q);
            IntelMat borderm = dst.channel(q);
...
static void copy_cut_border_image(const IntelMat& src, IntelMat& dst, int top, int left)
{
    int w = dst.w;
    int h = dst.h;
...
static void resize_bilinear_image(const IntelMat& src, IntelMat& dst, int w, int h)
{
    ...
    // loop body
    IntelMat rowsbuf0(w + 1);
    IntelMat rowsbuf1(w + 1);
...
void resize_bilinear(const IntelMat& src, IntelMat& dst, int w, int h)
{
    ...
    else if (src.dims == 3)
    {
        ...
        {
            const IntelMat m = src.channel(q);
            IntelMat resizem = dst.channel(q);
